...
const webpack_1 = require("./src/webpack");
...
class Server {
    ...
    configureAssets(distFolder, webpackConfig) {
        (0, webpack_1.applyWebpackHandlers)(distFolder, webpackConfig, this._express);
    }
    /**
     * Starts server
     *
    ...
    async _start(portType) {
        try {
            this._config = await sdk_config_1.default.init(this._blockPath, HEALTH_ENDPOINT, portType);
            ...
            if (err.message && err.message.indexOf('ECONN') > -1) {
                if (this._config) {
                    throw new Error('Failed while connecting to cluster server at ' +
